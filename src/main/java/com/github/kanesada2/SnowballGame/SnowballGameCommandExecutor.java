	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		switch (args.length) {
			case 0: {
				...
				msgs[2] = "/sbg get [Ball|Bat|Glove] <Highest|Higher|Normal|Lower|Lowest>" + ChatColor.YELLOW
						+ "get SnowballGame's custom item.";
			...
			case 1: {
				if (args[0].equalsIgnoreCase("reload")) {
					...
					for (Iterator<Entity> eitr = entities.iterator(); eitr.hasNext();) {
						...
						if (entity instanceof ArmorStand && entity.getCustomName() != null && entity.getCustomName()
								.equalsIgnoreCase(plugin.getConfig().getString("Coach.Coach_Name"))) {
							SnowballGameAPI.playWithCoach(player, (ArmorStand) entity,
									Util.getBallType(item.getItemMeta().getLore()));
							break;
					...
					new PlayerCoolDownTask(plugin, player).runTaskLater(plugin,
							plugin.getConfig().getInt("Ball.Cool_Time", 30));
					return true;
